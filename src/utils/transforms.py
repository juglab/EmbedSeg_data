...
class RandomRotationsAndFlips(T.RandomRotation):
    ...
    def __call__(self, sample):
        ...
        for idx, k in enumerate(self.keys):
            ...
            assert(k in sample)
            # NOTE sample[k] is Y x X (for example, 256 x 256)
            temp = np.ascontiguousarray(np.rot90(sample[k][np.newaxis, ...], times, (1, 2))[0])
            if flip==0:
                sample[k] = temp
            else:
                sample[k] = np.ascontiguousarray(np.flip(temp, axis=0)) # flip about y - axis
        return sample
    ...
    def __call__(self, sample):
        ...
        for idx, k in enumerate(self.keys):
            ...
            if t == torch.FloatTensor: # image
                sample[k] = torch.from_numpy(sample[k]).float().div(255) # TODO
            elif t ==torch.ByteTensor: # instance, label

                sample[k] = torch.from_numpy(sample[k]).byte()  # TODO
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for idx, k in enumerate(self.keys):
            ...
            print("as a numpy", sample[k].shape)
            t = self.type
            if isinstance(t, collections.Iterable):
                t = t[idx]
