...
class DSBReduced2018Dataset(Dataset):

    class_ids= (0)
    def __init__(self, root_dir='./', type="train", bg_id=0, size=None, transform=None): #TODO: 26

        print('DSB2018 Reduced Dataset created')

        # get image and instance list
        image_list=glob.glob(os.path.join(root_dir, 'dsbreduced2018/final-0/{}/'.format(type), 'images/*.tif'))
        image_list.sort()
        self.image_list = image_list

        instance_list=glob.glob(os.path.join(root_dir, 'dsbreduced2018/final-0/{}/'.format(type), 'masks/*.tif'))
        instance_list.sort()
