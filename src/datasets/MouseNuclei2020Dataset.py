...
class MouseNuclei2020Dataset(Dataset):

    def __init__(self, root_dir='./', type="train", bg_id=0, size=None, transform=None): #TODO: 26

        print('MouseNuclei2020 Dataset created')

        # get image and instance list
        image_list=glob.glob(os.path.join(root_dir, 'final-0/{}/'.format(type), 'images/*.tif'))
        image_list.sort()
        self.image_list = image_list

        instance_list=glob.glob(os.path.join(root_dir, 'final-0/{}/'.format(type), 'masks/*.tif'))
